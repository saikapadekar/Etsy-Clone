...
const deleteProductInShop = async (req, res) => {
    ...
    const product = await Product.findOne({ where: { id: productID, shopId: shopID } });
    if (!product) {
      res.status(404).json({ ...errors.notFound, message: 'product not found for shop' });
      return;
    }
